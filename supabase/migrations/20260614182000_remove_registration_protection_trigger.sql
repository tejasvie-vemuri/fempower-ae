...
DROP FUNCTION IF EXISTS public.protect_registration_fields();

CREATE OR REPLACE FUNCTION public.request_registration_cancellation(
...
BEGIN
  ...
  SET
    ...
    cancellation_reason = NULLIF(LEFT(TRIM(COALESCE(_reason, '')), 1000), '')
...
REVOKE ALL ON FUNCTION public.request_registration_cancellation(uuid, text)
  FROM PUBLIC, anon, authenticated;
GRANT EXECUTE ON FUNCTION public.request_registration_cancellation(uuid, text)
  TO authenticated;

CREATE OR REPLACE FUNCTION public.confirm_free_registration(
  _event_id uuid,
...
REVOKE ALL ON FUNCTION public.confirm_free_registration(uuid, jsonb, integer, jsonb)
  FROM PUBLIC, anon, authenticated;
GRANT EXECUTE ON FUNCTION public.confirm_free_registration(uuid, jsonb, integer, jsonb)
  TO authenticated;
