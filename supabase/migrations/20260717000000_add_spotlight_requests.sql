-- Spotlight Requests: admin invites a member to share her story via a guided form
...
CREATE TABLE public.spotlight_requests (
  ...
  advice            TEXT,
  shoutout          TEXT,
  photo_url         TEXT, -- storage path in the member-photos bucket, resolved via a signed URL
  ...
  -- Belt-and-suspenders: a request can't leave 'pending' without recorded consent,
  -- even if something bypasses the client-side form gate.
...
-- Structured fields on member_spotlights so the public card and the admin
-- preview can render the same guided-story layout instead of one free-text blob.
