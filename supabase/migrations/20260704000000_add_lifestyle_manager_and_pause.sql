...
-- Stage 1: full data model, standalone tables, no dependency on chat/AI/Google/WhatsApp yet.

-- ── Per-user Lifestyle Manager settings ──────────────────────────────
...
create policy "Members manage own lm_profile"
  on public.lm_profile for all
  using (auth.uid() = user_id)
  with check (auth.uid() = user_id);

-- ── Editable trial length + price (single row, admin managed) ───────
...
create policy "Admins can update plan_config"
  on public.plan_config for update
  using (public.has_role(auth.uid(), 'admin'))
  with check (public.has_role(auth.uid(), 'admin'));

-- ── People she cares about (the occasion memory core) ────────────────
...
create policy "Members manage own important_dates"
  on public.important_dates for all
  using (auth.uid() = user_id)
  with check (auth.uid() = user_id);

-- ── Shared cultural occasion calendar (admin maintained, everyone reads) ──
...
create policy "Anyone can read occasion_calendar"
  on public.occasion_calendar for select
  using (true);

create policy "Admins manage occasion_calendar"
  on public.occasion_calendar for all
  using (public.has_role(auth.uid(), 'admin'))
  with check (public.has_role(auth.uid(), 'admin'));

-- ── General reminders / tasks (chat-captured or manual) ──────────────
...
-- ── Curated restaurant / experience venues (ops maintained) ──────────
...
create policy "Anyone can read restaurant_venues"
  on public.restaurant_venues for select
  using (true);

create policy "Admins manage restaurant_venues"
  on public.restaurant_venues for all
  using (public.has_role(auth.uid(), 'admin'))
  with check (public.has_role(auth.uid(), 'admin'));

-- ── Cache of Google Places enrichment (rating, photos, hours) ────────
...
-- ── Booking / event requests, Google handoff model ───────────────────
create table if not exists public.booking_requests (
  ...
  status                text not null default 'suggested'
                        check (status in ('suggested', 'handed_off', 'self_confirmed', 'not_booked')),
  created_at            timestamptz not null default now()
);

alter table public.booking_requests enable row level security;

create policy "Members manage own booking_requests"
  on public.booking_requests for all
  using (auth.uid() = user_id)
  with check (auth.uid() = user_id);

-- ── Grocery list, organize-and-handoff model ─────────────────────────
...
create policy "Members manage own grocery_items"
  on public.grocery_items for all
  using (auth.uid() = user_id)
  with check (auth.uid() = user_id);

-- ── Chat history with Zoya (web + WhatsApp share one thread) ─────────
...
create policy "Members manage own chat_messages"
  on public.chat_messages for all
  using (auth.uid() = user_id)
  with check (auth.uid() = user_id);

-- ── Unified "confirm before I act" log, reused across every action type ──
...
create policy "Members manage own approval_actions"
  on public.approval_actions for all
  using (auth.uid() = user_id)
  with check (auth.uid() = user_id);

-- ── Google Calendar OAuth tokens (separate consent from site login) ──
...
create policy "Members manage own google_calendar_tokens"
  on public.google_calendar_tokens for all
  using (auth.uid() = user_id)
  with check (auth.uid() = user_id);

-- ── The Pause: personal journal (gratitude, one thing learned, free text) ──
...
create policy "Members manage own journal_entries"
  on public.journal_entries for all
  using (auth.uid() = user_id)
  with check (auth.uid() = user_id);

-- Adaptive, no-guilt nudge settings: widens automatically after two
-- consecutive misses, resets to daily the moment she writes an entry.
...
create policy "Members manage own journal_nudge_settings"
  on public.journal_nudge_settings for all
  using (auth.uid() = user_id)
  with check (auth.uid() = user_id);
